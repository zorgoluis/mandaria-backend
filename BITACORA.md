...
- **Estado funcional V1.10-A:** implementado y verificado el 2026-09-21: 130 unitarias, 186 E2E por archivo, `verify-migrations` en tres bases (incluida una llevada a V1.9 con datos reales), tormenta de concurrencia contra `dist/main.js` 7/7 y 0 violaciones del ledger en ambas bases. Queda el fallo preexistente de `delivery-quotes` (pool de Prisma, camino V1.6).
...
- **Definition of Done:** requisitos V1.10-A verificados localmente (ver VERIFICATION.md). Suite E2E completa limpia no obtenida en una sola corrida por la caída nativa de workers en Windows; cada archivo pasa por separado salvo el fallo preexistente de `delivery-quotes`. Docker sigue pospuesto por el propietario.
