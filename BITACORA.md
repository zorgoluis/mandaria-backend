...
- **Versión del paquete:** 1.10.0 (V1.10-A Credit Accounts & Immutable Ledger + V1.10-B Credit Policy Engine + V1.10-C Dispatch Credit Snapshot).
...
- **Objetivo actual:** V1.10-C: congelar en cada Dispatch nuevo el costo en créditos por actor (snapshot inmutable). Los créditos son enteros y no son dinero. **CLAIM y TAKE todavía no consumen créditos** (eso empieza en V1.10-D, no iniciado). No iniciados: V1.10-D en adelante, pagos online, Driver App. La «V1.9-C Service Coverage» que mencionó el propietario no existe en el repositorio; pendiente aclararlo.
...
- **Estado funcional V1.10-C:** implementado, verificado y **validado adversarialmente (CHECK, 33/33 por HTTP real)** el 2026-09-22 (sin commit ni push): al abrirse un Dispatch se congela, en la misma transacción que acepta la cotización, el costo en créditos por actor permitido (`PROVIDER` e `INDEPENDENT_DRIVER` para `LOCAL_DELIVERY`) con la política ACTIVE y `DeliveryQuote.distanceMeters`; fallo cerrado (409 `CREDIT_POLICY_UNAVAILABLE`), snapshots inmutables, cambio de política sin efecto retroactivo, Dispatches legacy sin costo inventado. **No cobra: CLAIM y TAKE no consumen créditos.** 158 unitarias, E2E 230/230 en 17 archivos, verify-migrations con V1.10-C, humo real 5/5 sobre `mandaria_db`, mutaciones 5/5; el CHECK no encontró ningún defecto del producto y probó la migración V1.10-B → V1.10-C con los datos reales copiados.
- **Estado funcional V1.10-B:** implementado, verificado y **validado adversarialmente (CHECK, 29/29 por HTTP real)** el 2026-09-22 (sin commit): políticas de créditos versionadas por serviceType + actorType (PER_KM, FLAT, DISTANCE_RANGE), máximo una ACTIVE, versionado atómico y concurrente, inmutabilidad en PostgreSQL y cálculo puro desde la distancia canónica. **Sólo calcula: CLAIM y TAKE no consumen créditos.** 147 unitarias, E2E 216/216 por archivo, verify-migrations con V1.10-A ledger → V1.10-B, humo real 14/14, mutaciones 6/6. Seed local de políticas iniciales aplicado en `mandaria_db`.
- **Estado funcional V1.10-A:** implementado, verificado y **validado adversarialmente (CHECK) el 2026-09-21**: 24/24 comprobaciones por HTTP real contra `dist/main.js`, 0 violaciones del ledger en ambas bases, 131 unitarias y 197 E2E. El CHECK encontró y corrigió 1 defecto real (interruptor de purga del ledger utilizable fuera de bases `_test`; migración `20260921001200`). Implementado originalmente en el commit `533bf10`. El mismo día se corrigió el defecto de cotizaciones concurrentes de V1.6 que causaba el único fallo arrastrado de la suite: 131 unitarias y **197 E2E con 15/15 archivos en verde, sin fallos**, `verify-migrations` en tres bases, tormenta de créditos 7/7 y 0 violaciones del ledger.
...
- **Contrato OpenAPI:** desde el 2026-09-21 ningún campo de `docs/openapi.json` se publica como `type: object` sin estructura (auditoría en 0; antes 130 anulables de V1.1–V1.9). Cambio sólo de documentación, commiteado en `v1.10-credit-monetization` sin push.
...
### 2026-09-21 — OpenAPI: tipos explícitos en 130 campos anulables (sólo documentación)

- **Solicitud:** eliminar los 130 campos publicados en OpenAPI como `type: object` sin estructura, sin cambiar comportamiento. Auditoría objetivo en 0. No commit, no push.
- **Causa:** propiedades `campo!: string | null` (o `Date`/`number`/objeto anidado) con `@ApiPropertyOptional({ nullable: true })` sin `type`: `design:type` refleja la unión como `Object`. Mismo patrón ya aplicado en `src/credits/credits.responses.ts`.
- **Cambios:** 103 decoradores en 12 archivos (`*.responses.ts` y DTOs de delivery-requests, vehicles, independent-drivers e integrations) reciben `type` explícito: `String` en textos y UUID (se conserva `format: 'uuid'`), `String` + `date-time` en marcas de tiempo, `'integer'` en `year` (`@IsInt`) y `Number` en `weightKg`/`lengthCm`/`widthCm`/`heightCm` (decimales). Tres objetos anidados libres pasan a clases nuevas: `DriverDispatchZoneResponse` (`code`, `name`), `DriverAssignmentVehicleResponse` (`id`, `identifier`, `type`, `plate`) y `DeliveryAssignmentProviderResponse` (`id`, `name`, `code`), con los campos que seleccionan los servicios. Validadores, selects y payloads intactos. `MyIndependentProfileResponse.activeAssignment` sigue sin tocarse: ningún controlador la usa y no se publica.
- **Verificaciones ejecutadas (2026-09-21):** `docs:openapi` y auditoría en 0 (antes 130); comparación con `HEAD:docs/openapi.json`: 120/120 rutas, operaciones idénticas, 0 esquemas y 0 propiedades eliminados, `required` sin cambios, +3 esquemas nuevos; los 130 cambios son `object`→`string` (110), `number` (10), `integer` (7) y referencia (3). `docs:check`, `tsc --noEmit`, Oxlint, ESLint y `build` limpios; 130 unitarias PASS sobre `533bf10`. E2E (a petición del propietario) sobre `533bf10`: por archivo 196/197, con el único fallo de `delivery-quotes` que corrige `38d9b5d`. **Tras rebasar sobre `38d9b5d`:** auditoría en 0 y el mismo diff semántico, `docs:check`, tsc, Oxlint, ESLint y build limpios, 131 unitarias PASS; E2E por archivo 14/15 PASS (incluido `delivery-quotes` 11/11). `independent-drivers` 7/23 por datos contaminados en `mandaria_test`: una `DeliveryAssignment` ACTIVE (FLEET) que dejó un worker de `delivery-assignments` caído a mitad de prueba (caída nativa de Windows), sin relación con este cambio. La misma prueba pasó 23/23 con este código antes del rebase. Por autorización del propietario se borró sólo esa fila de `mandaria_test` (1 fila; 0 asignaciones ACTIVE después) y `independent-drivers` pasó 23/23: **E2E 15/15 archivos, 197/197**.
- **Resultado:** contrato OpenAPI sin campos ambiguos. Commit en `v1.10-credit-monetization` sin push (instrucción del propietario).
- **Pendientes:** actualizar el cliente/contrato de `mandaria-frontend` si genera tipos desde OpenAPI; resto de pendientes de V1.10-A sin cambios.

### 2026-09-21 — CHECK V1.10-A (Credit Accounts & Immutable Ledger)

- **Solicitud:** intentar romper cuentas, ledger, idempotencia, atomicidad y concurrencia contra backend y PostgreSQL reales antes de que V1.10-B/V1.10-D dependan de ellos. Sin V1.10-B, sin cobrar CLAIM/TAKE, sin commit ni push.
- **Entorno encontrado:** cliente Prisma del 17-sep y ambas bases en V1.8 (V1.9 y V1.10 nunca aplicadas en esta máquina). Se regeneró el cliente, se respaldaron `mandaria_db` y `mandaria_test` con `pg_dump` (en el scratchpad de la sesión) y se aplicaron las migraciones sin reset; las 23 tablas previas conservaron filas y contenido (hash por tabla sobre sus columnas originales).
- **Validación real:** validador temporal fuera del repositorio, servidor `dist/main.js` sobre `mandaria_test` con reinicios para no agotar el límite de 100 peticiones/minuto ni el de 5 logins/minuto: **24/24 PASS** (propiedad, saldos iniciales, recarga, recargas inválidas, ajustes, saldo negativo, atomicidad con fallos inyectados antes y después de mover el saldo, concurrencia -8∥-8 y +100∥+200∥+300, 60 movimientos simultáneos, idempotencia incluidas 5 copias simultáneas, inmutabilidad, 19 restricciones, matemática del ledger, matriz de autorización, aislamiento, suspensión, ausencia de DELETE, créditos sin moneda, regresión CLAIM/TAKE con saldo 0, OpenAPI y logs sin secretos).
- **Bug real corregido:** el interruptor `mandaria.ledger_purge` permitía borrar el ledger en cualquier base y con cualquier rol con permiso DELETE (demostrado con un rol temporal sin privilegios de dueño y, en `mandaria_db`, dentro de una transacción revertida). Migración `20260921001200_credit_ledger_purge_test_only`: sólo en bases cuyo nombre termina en `_test`. `verify-migrations` lo comprueba; README corregido (su mitigación anterior no cerraba el hueco).
- **Documentación:** la paginación compartida (21 respuestas + parámetros `page`/`pageSize`) pasa de `number` a `integer` en OpenAPI; sin cambios de comportamiento.
- **Regresión y calidad:** prisma validate, migrate status (ambas bases), verify-migrations, build, tsc, Oxlint, ESLint, Prettier y docs:check PASS; 131 unitarias; E2E 197/197 por archivo (2 caídas nativas de workers, verdes al repetir).
- **Limpieza:** fixtures del CHECK sin restos; eliminados de `mandaria_test` los restos de corridas de `credits` interrumpidas por la caída de workers (5 proveedores, 20 movimientos). Las 12 cuentas de migración de `mandaria_db` intactas.
- **Riesgos restantes:** el dueño de las tablas aún puede desactivar triggers (producción debe usar un rol que no sea dueño y una base que no termine en `_test`); un rol con INSERT puede escribir entradas SERVICE_* bien formadas sin actor (camino reservado a V1.10-D); la Idempotency-Key es por cuenta; recargas son declaraciones sin conciliación bancaria; caída nativa de workers en Windows.

### 2026-09-22 — V1.10-B Credit Policy Engine

- **Solicitud:** motor configurable y versionado de cuántos créditos cuesta adjudicarse un servicio (serviceType + actorType + distancia canónica), sólo cálculo: sin débito, sin tocar CLAIM/TAKE, sin snapshot en el Dispatch (V1.10-C), sin refunds ni web. Sin commit/push.
- **Decisiones:** `actorType` reutiliza `CreditAccountOwnerType` (PROVIDER, INDEPENDENT_DRIVER: exactamente quién paga); estados ACTIVE/INACTIVE; toda versión nace ACTIVE y reemplaza a la anterior en la misma transacción; `effectiveFrom`/`effectiveUntil` los fija el servidor (sin scheduler ni activación futura); versionar sólo desde la ACTIVE (si no, 409 `CREDIT_POLICY_VERSION_CONFLICT`) con bloqueo consultivo por combinación (namespace 71_600_020); rangos `[min, max)` como las bandas V1.6, desde 0, contiguos y con el último abierto; límites de créditos = límite de movimiento V1.10-A (1 000 000); aritmética entera; sin política → `CREDIT_POLICY_UNAVAILABLE` (nunca 0); INACTIVE no se reactiva; sin DELETE/PATCH; sólo SUPER_ADMIN (proveedores y repartidores no leen reglas).
- **Prisma / migración:** enums `CreditCalculationType`, `CreditPolicyStatus`; modelos `CreditPolicy`, `CreditPolicyRange`. Migración `20260922001300_credit_policies`: únicos, índice parcial de una ACTIVE, CHECKs (con `IS NOT NULL` explícitos), triggers de versión máx + 1 / nacimiento ACTIVE / inmutabilidad / sin DELETE ni TRUNCATE (salvo el interruptor `_test`) y trigger diferido de rangos completos. No crea políticas.
- **API:** `GET /admin/credit-policies`, `GET /admin/credit-policies/:id`, `POST /admin/credit-policies`, `POST /admin/credit-policies/:id/versions`, `GET /admin/credit-policies/calculation`. Interfaz interna para V1.10-C/D: `CreditPoliciesService.resolveActivePolicy()` / `calculate()` y la función pura `calculateCreditCost()`.
- **Política inicial:** seed LOCAL/TEST ONLY `db:seed:local-credit-policies` (idempotente, nunca edita), incluido en `db:seed:local`; ejecutado en `mandaria_db` (v1 PER_KM 1/3 para ambos actores, atribuidas al SUPER_ADMIN de bootstrap). Producción: SUPER_ADMIN por API.
- **Hallazgos propios corregidos:** trigger diferido con `CASE` que referenciaba un campo inexistente de `NEW` (retirada y reaplicación local de la migración aún no commiteada, con tablas vacías); `distanceMeters` vacío convertido en 0; CHECK que habría aceptado NULL. En pruebas, supertest cerraba el servidor compartido entre peticiones concurrentes.
- **Verificaciones:** ver VERIFICATION.md (147 unitarias, 216 E2E, verify-migrations, humo real 14/14, 28 ataques SQL rechazados, mutaciones 6/6, ledger intacto).
- **Documentación:** README (sección Credit Policy Engine, scripts, riesgos, alcance), OpenAPI/API_ACCESS regenerados, `mandaria-frontend/docs/API-CONTRACT.md` (extensión V1.10-B, sólo documentación).
- **Pendientes:** V1.10-C (snapshot del costo en el Dispatch con la distancia canónica congelada), V1.10-D (cobro en CLAIM/TAKE; decidir el caso de costo 0 con `minimumCredits: 0`), endpoint para deshabilitar una combinación si se necesita.

### 2026-09-22 — CHECK V1.10-B (Credit Policy Engine)

- **Solicitud:** validar V1.10-B contra backend y PostgreSQL reales sin implementar V1.10-C ni consumir créditos. Sin commit/push.
- **Validación real:** validador temporal contra `dist/main.js` sobre `mandaria_test`: **29/29 PASS** (matriz por actor, PER_KM exacto incluido 3001 → 4, tarifa 2/km → 14, FLAT, fronteras de rangos sin huecos ni doble coincidencia, rangos inválidos, historial e inmutabilidad por API y SQL, 12 versiones simultáneas y cadenas concurrentes, carreras de activación en SQL, fallo cerrado sin política, aislamiento de actores, sin llamadas a routing con Google configurado con una clave inutilizable, distancias y enteros inválidos, desbordamiento, autorización, campos falsificados, 16 invariantes SQL, cuentas y ledger intactos, CLAIM/TAKE con saldo 0, cambio de política con operaciones vivas, OpenAPI).
- **Calidad y regresión:** prisma validate, migrate status, sin drift, verify-migrations (V1.10-A ledger → V1.10-B), build, tsc, Oxlint, ESLint, docs:check; 147 unitarias; E2E 216/216.
- **Bugs del producto:** ninguno; sin cambios de código. Tres errores del propio validador corregidos en el validador.
- **Incidente:** una limpieza con el prefijo genérico `E2E_` borró por error restos de corridas E2E anteriores en `mandaria_test` (1 300 DeliveryRequest y dependientes, zonas `E2E_`); `mandaria_db` intacta; E2E repetida en verde después.
- **Limpieza:** `mandaria_test` con 0 políticas y 0 fixtures del CHECK; `mandaria_db` con sus políticas operativas v1 PER_KM 1/3 para ambos actores sin cambios.
- **Riesgos restantes:** los de V1.10-B (costo 0 con `minimumCredits: 0`, sin endpoint para deshabilitar una combinación, sin activación programada); un escritor SQL directo que respete las reglas de la base puede crear una versión fuera del bloqueo de la API (la base sigue garantizando una ACTIVE y versiones contiguas).

### 2026-09-22 — V1.10-C Dispatch Credit Snapshot

- **Solicitud:** congelar al abrir el Dispatch el costo en créditos por actor permitido del ServiceType, con la política ACTIVE y la distancia canónica; sin consumir créditos, sin tocar la economía de CLAIM/TAKE, sin SERVICE_AWARD/REFUND ni UI. Sin commit/push.
- **Decisiones:** actores desde `SERVICE_EXECUTION_MODES` (espejo SQL `credit_required_actors()`); distancia = `DeliveryQuote.distanceMeters` (sin nueva llamada de routing); snapshots creados en `openDispatch()` dentro de la transacción de aceptación; bloqueo consultivo compartido por actor en el namespace 71_600_020 (exclusivo en el versionado); evidencia completa por tipo de cálculo; créditos 1–1 000 000 (costo 0 → 422 `CREDIT_COST_OUT_OF_RANGE`); sin política → 409 y la cotización sigue OFFERED; legacy sin backfill (`creditCost: null`, `creditSnapshots: []`, `legacyWithoutCreditSnapshots: true`); proveedor y repartidor ven sólo su `creditCost`, SUPER_ADMIN todos, B2B nada.
- **Prisma / migración:** modelo `DispatchCreditSnapshot`; migración `20260922001400_dispatch_credit_snapshots` (único por Dispatch + actor, CHECK de valores y evidencia, guardián que recalcula el costo en SQL y exige Dispatch OPEN recién creado, inmutabilidad, DELETE sólo en cascada o purga `_test`, sin TRUNCATE, trigger diferido que exige todos los snapshots al COMMIT). Aplicada en `mandaria_db` y `mandaria_test` sin reset ni cambios de datos (0 backfill).
- **Hallazgo propio corregido:** el guardián no trataba la política inexistente (`NOT FOUND`); retirada y reaplicación local de la migración aún no commiteada (tabla vacía en `mandaria_db`).
- **Pruebas:** `test/dispatch-credit-snapshots.spec.ts` (10) y `.e2e-spec.ts` (14); `dispatch.spec.ts` ampliado. E2E en serie (`fileParallelism: false`) con política base `test/support/credit-policies.ts` en las suites que aceptan cotizaciones.
- **Verificaciones:** ver VERIFICATION.md.
- **Documentación:** README (sección Dispatch Credit Snapshot, riesgos, alcance), OpenAPI regenerado, `mandaria-frontend/docs/API-CONTRACT.md` (extensión V1.10-C, sólo documentación).
- **Pendientes:** V1.10-D (cobro al CLAIM/TAKE usando el snapshot; decidir el trato de los Dispatches legacy sin snapshot); crear las políticas antes de aceptar cotizaciones en cada entorno nuevo.

### 2026-09-22 — CHECK V1.10-C (Dispatch Credit Snapshot)

- **Solicitud:** validar V1.10-C contra backend y PostgreSQL reales sin implementar V1.10-D ni debitar créditos. Sin commit/push.
- **Validación real:** validador temporal contra `dist/main.js` sobre `mandaria_test`: **33/33 PASS** (dos actores con políticas distintas y 6240 m → 7 y 14 en DB y API, aislamiento de vistas, inmutabilidad ante cambio de política, Dispatch nuevo 21/28, versión histórica exacta, 6 ataques de UPDATE/DELETE/TRUNCATE, duplicados, datos inválidos, actor cruzado, fallo cerrado por cada actor con reintento exitoso, rollback del 2.º snapshot por servidor y por SQL, 12 aperturas contra 2 versionados, reintentos sin duplicados, 0 llamadas extra de routing, FLAT, DISTANCE_RANGE interpretable tras cambiar la política, distancia 0, cuentas y ledger intactos, CLAIM/TAKE con saldo 0, contexto de pago separado, aislamiento B2B, legacy sin costo inventado, migración con datos reales, escaneo de invariantes, OpenAPI y limpieza).
- **Migración:** base temporal con esquema V1.10-B y los datos reales de `mandaria_db`; aplicar sólo la migración V1.10-C dejó las 28 tablas con las mismas filas y hashes, 44 Dispatches legacy, 0 snapshots y sin drift. Base y volcado eliminados.
- **Calidad y regresión:** prisma validate, migrate status (ambas), sin drift, verify-migrations, build, tsc, Oxlint, ESLint, docs:check; 158 unitarias; E2E 230/230 en 17 archivos.
- **Bugs del producto:** ninguno; sin cambios de código. Dos errores del propio validador corregidos en el validador (marca de tiempo local como `effectiveUntil` en columnas UTC; contador de limpieza).
- **Limpieza:** `mandaria_test` sin fixtures ni políticas del CHECK y con la política base ACTIVE restaurada; `mandaria_db` idéntica antes y después (sólo lectura).
- **Riesgos restantes:** los de V1.10-C (crear políticas antes de aceptar cotizaciones, Dispatches legacy sin costo para V1.10-D, actores requeridos duplicados en código y SQL); una combinación puede quedarse sin política ACTIVE sólo por SQL directo y no hay endpoint para reactivarla; el dueño de las tablas puede desactivar triggers (en producción usar un rol que no sea dueño y una base que no termine en `_test`).
