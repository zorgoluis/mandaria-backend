...
- **Versión del paquete:** 1.9.0 (V1.9-A Independent Drivers).
- **Rama activa:** `v1.9-independent_drivers`, creada por el propietario tras el merge de V1.8 (PR #13). V1.9 sin commit ni push (instrucción del propietario).
- **Repositorio remoto:** https://github.com/zorgoluis/mandaria-backend.git. Entrega V1.2 en `V1_2-Proveedores_Reparto`.
- **Objetivo actual:** V1.9-A: segundo modelo de ejecución. Un Driver existente habilitado por SUPER_ADMIN toma servicios por su cuenta (`IndependentDriverProfile`, vehículos propios, `take` = claim + assignment atómico y `release`), conviviendo con el modelo de flotilla V1.7/V1.8 sobre la misma `DeliveryAssignment`. Fuera de alcance y no iniciados: Driver App, autorregistro, verificación documental, GPS, tracking, sockets, wallet y créditos.
...
- **Definition of Done:** requisitos V1.9-A verificados localmente y validados adversarialmente (ver VERIFICATION.md). Suite E2E completa limpia no obtenida en una sola corrida por la caída nativa de workers en Windows; cada archivo pasa por separado. El fallo de `delivery-quotes` es preexistente y de entorno, no de V1.9. Docker sigue pospuesto por el propietario.
