...
- **Versión del paquete:** 1.8.0 (V1.8-A Provider Driver & Vehicle Assignment).
- **Rama activa:** `v1.8-provider_driver_vehicle_assignment`, creada por el propietario tras el merge de V1.7 (PR #12). V1.8 sin commit ni push (instrucción del propietario).
- **Repositorio remoto:** https://github.com/zorgoluis/mandaria-backend.git. Entrega V1.2 en `V1_2-Proveedores_Reparto`.
- **Objetivo actual:** V1.8-A: tras el claim, el proveedor asigna Driver y Vehicle propios con historial (DeliveryAssignment ACTIVE/REASSIGNED/CANCELLED), reasignación, plazo de asignación y contexto de pago. V1.9 (Driver App, Drivers independientes) no iniciado; sin aceptación del repartidor, GPS, tracking, sockets ni wallet.
...
- **Estado funcional V1.8-A:** implementado, verificado y validado adversarialmente (CHECK) el 2026-09-17; 91 pruebas unitarias; E2E 148/148 ejecutando cada archivo por separado (la caída nativa conocida de workers en Windows apareció en 1 archivo y desapareció al repetirlo, sin fallos reales); mutaciones 10/10 detectadas; verify-migrations V1.0 → V1.8 PASS; CHECK V1.8-A 30/30 por HTTP real contra `dist/main.js` y 0 violaciones en el escaneo de ambas bases.
- **Definition of Done:** requisitos V1.8-A verificados localmente (ver VERIFICATION.md). Suite E2E completa limpia no obtenida en una sola corrida por la caída nativa de Windows; cada archivo pasa. Docker sigue pospuesto por el propietario.
